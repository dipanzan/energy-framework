...
static void print_energy_consumed(long long before, long long after, const char *exp)
{
	double result = (after - before) / pow(10, 6) /* * PERF_CONSTANT */;
	// printf("=====================KERNEL-ENERGY-FRAMEWORK======================\n");
	printf("%s(): %f\n", exp, result);
	// printf("||                                                              ||\n");
	// printf("=====================KERNEL-ENERGY-FRAMEWORK======================\n");
}
...
static void stress_syscall_report_syscall_top10(const stress_args_t *args)
{
	...
	for (i = 0; i < STRESS_SYSCALLS_MAX; i++)
	{
		...
		if (ss->succeed)
		{
			ss->average_duration = (ss->count > 0) ? ss->total_duration / (double)ss->count : SYSCALL_DAY_NS;
		}
		else
		{
...
static void stress_syscall_benchmark_calls(const stress_args_t *args)
{
	...
	for (i = 0; i < STRESS_SYSCALLS_MAX; i++)
	{
		...
		long before = read_energy(1);
		ret = syscalls[j].syscall();
		long after = read_energy(1);
		print_energy_consumed(before, after, syscalls[j].name);
...
static int stress_syscall(const stress_args_t *args)
{
	...
	int syscall_method = SYSCALL_METHOD_ALL;
